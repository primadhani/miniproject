<x-app-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Tambah Modul Baru untuk: ') . $materi->nama_materi }}</h2>
        <div class="text-sm mt-1">
            <a href="{{ route('admin.materi.index') }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Manajemen Materi') }}</a>
            / <a href="{{ route('admin.materi.modul.index', $materi->id_materi) }}" class="text-indigo-600 hover:text-indigo-900">Modul</a> / Tambah
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.materi.modul.store', $materi->id_materi) }}">
                        @csrf
                        <div class="mb-4">
                            <x-input-label for="nama_modul" :value="__('Nama Modul')" />
                            <x-text-input id="nama_modul" name="nama_modul" type="text" class="mt-1 block w-full" :value="old('nama_modul')" required autofocus />
                        ...
                        <div class="flex items-center justify-end">
                            ...
                            <x-primary-button>{{ __('Simpan Modul') }}</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
