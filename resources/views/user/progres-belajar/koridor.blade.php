<x-app-layout>
    <x-slot name="header">
        ...
        <div class="text-sm mt-1 text-gray-600">
            <a href="{{ route('user.proses-belajar') }}" class="hover:text-gray-900">{{ __('Proses Belajar') }}</a>
            
            @if (isset($learningPath))
                / <a href="{{ route('user.proses-belajar', $learningPath->id) }}" class="hover:text-indigo-900">
                    {{ $learningPath->nama_path ?? 'Learning Path' }}
                </a>
            @endif
            
            / <span class="text-gray-400">{{ $materi->nama_materi }}</span>
        </div>
    </x-slot>
